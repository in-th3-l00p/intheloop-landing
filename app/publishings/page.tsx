...
export default async function PortfolioPage() {
        ...
        <Section icon="❦" label="Projects" meta={`${pad2(projects.length)} works · ${settings.copyrightYear}`}>
          ...
          <div className="il-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "38px 28px", marginTop: 42 }}>
            {projects.map((p) => {
              const tile = (
                <>
                  <div style={{ position: "relative", aspectRatio: "4/5", overflow: "hidden", border: "1px solid rgba(230,230,226,.16)", background: "#15181b" }}>
                    <div className="il-projimg" style={{ width: "100%", height: "100%", backgroundImage: "repeating-linear-gradient(135deg,#1e2329 0 9px,#14181c 9px 18px)", position: "relative" }}>
                      <span style={{ position: "absolute", top: 12, left: 12, width: 11, height: 11, borderLeft: "1px solid rgba(230,230,226,.42)", borderTop: "1px solid rgba(230,230,226,.42)" }} />
                      <span style={{ position: "absolute", bottom: 12, right: 12, width: 11, height: 11, borderRight: "1px solid rgba(230,230,226,.42)", borderBottom: "1px solid rgba(230,230,226,.42)" }} />
                      <span style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, color: "#55606a" }}>＋</span>
                      <span style={{ position: "absolute", left: 13, bottom: 12, fontFamily: MONO, fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "#79838b" }}>img / {p.kind}</span>
                    </div>
                  </div>
                  ...
                  {p.summary ? (
                    <div style={{ fontFamily: MONO, fontSize: 12, lineHeight: 1.7, color: "#b4b9bf", marginTop: 8 }}>{p.summary}</div>
                  ) : null}
                </>
              );
              return p.href ? (
          ...
          <div className="il-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "32px 28px", marginTop: 42 }}>
            {caseStudies.map((c) => (
              <Link key={c.slug} href={`/case-study/${c.slug}`} data-reveal="" className="il-project" style={{ display: "block", textDecoration: "none", color: "inherit" }}>
                <div style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden", border: "1px solid rgba(230,230,226,.16)", background: "#15181b" }}>
                  <div className="il-projimg" style={{ width: "100%", height: "100%", backgroundImage: "repeating-linear-gradient(135deg,#1e2329 0 11px,#14181c 11px 22px)", position: "relative" }}>
                    <span style={{ position: "absolute", top: 12, left: 12, width: 11, height: 11, borderLeft: "1px solid rgba(230,230,226,.42)", borderTop: "1px solid rgba(230,230,226,.42)" }} />
                    <span style={{ position: "absolute", bottom: 12, right: 12, width: 11, height: 11, borderRight: "1px solid rgba(230,230,226,.42)", borderBottom: "1px solid rgba(230,230,226,.42)" }} />
                    <span style={{ position: "absolute", left: 13, bottom: 12, fontFamily: MONO, fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "#79838b" }}>case film / {c.kind}</span>
                  </div>
                </div>
    ...
    },
    {
      ...
      content: (
        <Section icon="❧" label="Articles" meta={`${pad2(articles.length)} pieces`}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 1, marginTop: 42, borderTop: "1px solid rgba(230,230,226,.14)" }}>
            {articles.map((a) => (
              <Link key={a.slug} href={`/articles/${a.slug}`} data-reveal="" className="il-row il-artrow" style={{ display: "grid", gridTemplateColumns: "160px 1fr auto", gap: 28, alignItems: "baseline", padding: "30px 8px", textDecoration: "none", color: "inherit", borderBottom: "1px solid rgba(230,230,226,.12)" }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: MONO, fontSize: 12, lineHeight: 1.8, color: "#b4b9bf", marginTop: 10, maxWidth: "60ch" }}>{a.summary}</div>
                </div>
  ...
  return (
    <Frame>
      ...
      {/* footer */}
      <div className="il-px" style={{ position: "relative", padding: "80px 40px 72px", maxWidth: 1320, margin: "72px auto 0", borderTop: "1px solid rgba(230,230,226,.15)" }}>
        <Link href="/" data-reveal="" className="il-card" style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", textDecoration: "none", color: "inherit", border: "1px solid rgba(230,230,226,.16)", background: "#15181b", overflow: "hidden", padding: "34px 34px 28px", minHeight: 172 }}>
          <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".16em", textTransform: "uppercase", color: "#97a0a7" }}>← Back</div>
          <div>
            <div style={{ fontSize: "clamp(30px,3.4vw,44px)", fontWeight: 400, letterSpacing: "-.015em", lineHeight: 1 }}>Home</div>
            <div style={{ fontFamily: MONO, fontSize: 12, color: "#b4b9bf", marginTop: 12 }}>The studio</div>
          </div>
        </Link>
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        marginTop: 42,
        border: "1px solid rgba(230,230,226,.16)",
        background: "#15181b",
        overflow: "hidden",
        padding: "92px 40px",
        display: "flex",
    ...
    >
      <div style={{ position: "absolute", inset: 0, backgroundImage: "repeating-linear-gradient(135deg,#1e2329 0 9px,#14181c 9px 18px)", opacity: 0.4 }} />
      <span style={{ position: "absolute", top: 14, left: 15, width: 12, height: 12, borderLeft: "1px solid rgba(230,230,226,.34)", borderTop: "1px solid rgba(230,230,226,.34)" }} />
