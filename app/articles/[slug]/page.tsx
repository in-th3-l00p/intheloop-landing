  ...
  return (
    <Frame>
      ...
      <div className="il-px il-secpad" style={{ position: "relative", padding: "110px 40px 56px", maxWidth: 1320, margin: "0 auto", overflow: "hidden" }}>
        ...
        {article.summary ? (
          <p data-reveal="" style={{ position: "relative", fontSize: "clamp(19px,2vw,25px)", lineHeight: 1.55, color: "#d2d6da", maxWidth: "44ch", marginTop: 26, fontStyle: "italic" }}>
            {article.summary}
          </p>
        ) : null}
        <div data-reveal="" style={{ display: "flex", flexWrap: "wrap", gap: 28, marginTop: 36, fontFamily: MONO, fontSize: 11, letterSpacing: ".06em", color: "#79838b" }}>
      ...
      {/* footer */}
      <div className="il-px" style={{ position: "relative", maxWidth: 1320, margin: "72px auto 0", padding: "56px 40px 72px", borderTop: "1px solid rgba(230,230,226,.15)" }}>
        <div className="il-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 24 }}>
          <Link href="/publishings#articles" data-reveal="" className="il-card" style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", textDecoration: "none", color: "inherit", border: "1px solid rgba(230,230,226,.16)", background: "#15181b", overflow: "hidden", padding: "30px 30px 24px", minHeight: 156 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: MONO, fontSize: 12, color: "#b4b9bf", marginTop: 10 }}>Back to the index</div>
            </div>
          </Link>
          {next ? (
            <Link href={`/articles/${next.slug}`} data-reveal="" className="il-card" style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", textDecoration: "none", color: "inherit", border: "1px solid rgba(230,230,226,.16)", background: "#15181b", overflow: "hidden", padding: "30px 30px 24px", minHeight: 156 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: MONO, fontSize: 12, color: "#b4b9bf", marginTop: 10 }}>{next.kind} ↗</div>
              </div>
            </Link>
          ) : (
            <Link href="/#contact" data-reveal="" className="il-card" style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "space-between", textDecoration: "none", color: "inherit", border: "1px solid rgba(230,230,226,.16)", background: "#15181b", overflow: "hidden", padding: "30px 30px 24px", minHeight: 156 }}>
              <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".16em", textTransform: "uppercase", color: "#97a0a7" }}>Inquiries →</div>
              <div>
                <div style={{ fontSize: "clamp(26px,3vw,38px)", fontWeight: 400, letterSpacing: "-.015em", lineHeight: 1 }}>Get in touch</div>
                <div style={{ fontFamily: MONO, fontSize: 12, color: "#b4b9bf", marginTop: 10 }}>The studio ↗</div>
              </div>
            </Link>
          )}
        </div>
...
function Block({ type, text }: { type: string; text: string }) {
  if (type === "heading") {
    return (
      <h2 data-reveal="" style={{ fontSize: "clamp(26px,3.2vw,38px)", fontWeight: 400, letterSpacing: "-.01em", lineHeight: 1.2, color: "#e6e6e2", margin: "52px 0 4px" }}>
        {text}
      </h2>
    );
  }
  if (type === "quote") {
    return (
      <blockquote data-reveal="" style={{ borderLeft: "2px solid #3f4a52", padding: "6px 0 6px 26px", margin: "40px 0" }}>
        <div style={{ fontSize: "clamp(22px,2.6vw,30px)", fontStyle: "italic", fontWeight: 400, lineHeight: 1.4, color: "#e6e6e2" }}>{accent(text)}</div>
      </blockquote>
    );
  }
  if (type === "code") {
    return (
      <pre data-reveal="" style={{ fontFamily: MONO, fontSize: 13, lineHeight: 1.8, color: "#c8cdd2", background: "#15181b", border: "1px solid rgba(230,230,226,.14)", borderRadius: 0, padding: "20px 22px", margin: "32px 0", overflowX: "auto" }}>
        <code>{text}</code>
      </pre>
    );
  }
  return (
    <p data-reveal="" style={{ fontSize: 18, lineHeight: 1.9, color: "#c6cbd0", margin: "24px 0" }}>
      {accent(text)}
    </p>
  );
}
